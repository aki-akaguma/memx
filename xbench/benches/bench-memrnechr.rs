...
#[allow(dead_code)]
#[inline(never)]
fn print_statistics_std_memrnechr(texts: &[&str], pat_byte: u8) {
    let map = statistics_std_memrnechr(texts, pat_byte);
...
fn criterion_benchmark(c: &mut Criterion) {
    ...
    if let Ok(_val) = std::env::var("AKI_TEST_DAT_CHECK") {
        ...
        if cpuid_avx2::get() {
            let n = process_memx_memrnechr_avx2(black_box(&vv), black_box(pat_byte));
            assert_eq!(n, match_cnt);
        }
        std::process::exit(0);
    }
    ...
    if let Ok(_val) = std::env::var("AKI_TEST_STATISTICS") {
        print_statistics_std_memrnechr(black_box(&vv), black_box(pat_byte));
        std::process::exit(0);
    }
    //
    cache_flush(&vv);

...
fn test00() {
    let buf = vec![];
    let r = test_memnechr_dbl(&buf, b'g', b'Z');
    assert_eq!(r, None);
    //
    let buf = vec![b'A', b'g'];
    ...
    for _ in 0..2 {
        vv.rotate_right(1);
        let r = test_memnechr_dbl(&buf, vv[0], vv[1]);
        assert_eq!(r, Some(1));
    }
    //
    ...
    let r = test_memnechr_dbl(&buf, b'A', b'B');
    assert_eq!(r, None);
}
#[test]
fn test01() {
...
fn test02() {
    ...
    let f = |x: usize| {
        ...
        any(target_arch = "x86_64", target_arch = "x86")) {
            x86_alignment_check::ac_call_once(|| { test_memnechr_dbl(&buf[x..], 0_u8, b'Z') })
        } else {
            test_memnechr_dbl(&buf[x..], 0_u8, b'Z')
        });

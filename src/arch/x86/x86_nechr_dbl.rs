...
#[inline(always)]
pub(crate) fn _memnechr_dbl_impl(buf: &[u8], needle: B1Dbl) -> Option<usize> {
...
unsafe fn _memnechr_dbl_basic(buf: &[u8], needle: B1Dbl) -> Option<usize> {
    basic::_memnechr_dbl_impl(buf, needle)
}

#[cfg(any(target_arch = "x86_64", target_arch = "x86"))]
#[target_feature(enable = "sse2")]
#[allow(clippy::missing_safety_doc)]
pub unsafe fn _memnechr_dbl_sse2(buf: &[u8], needle: B1Dbl) -> Option<usize> {
    _memnechr_dbl_sse2_impl(buf, needle)
}

#[cfg(any(target_arch = "x86_64", target_arch = "x86"))]
#[target_feature(enable = "avx2")]
#[allow(clippy::missing_safety_doc)]
pub unsafe fn _memnechr_dbl_avx2(buf: &[u8], needle: B1Dbl) -> Option<usize> {
    _memnechr_dbl_avx2_impl(buf, needle)
}

#[inline(always)]
fn _memnechr_dbl_sse2_impl(buf: &[u8], needle: B1Dbl) -> Option<usize> {
    let buf_len = buf.len();
    ...
    if buf_len >= 16 {
        ...
        // to a aligned pointer
        {
            if !buf_ptr.is_aligned_u128() {
                ...
                {
                    let r = basic::_nechr_dbl_to_aligned_u128(buf_ptr, needle, start_ptr);
                    if let Some(p) = r.0 {
                        buf_ptr = p;
    ...
    basic::_memnechr_dbl_remaining_15_bytes_impl(buf_ptr, cc, start_ptr, end_ptr)
}

#[inline(always)]
fn _memnechr_dbl_avx2_impl(buf: &[u8], needle: B1Dbl) -> Option<usize> {
    let buf_len = buf.len();
    let mut buf_ptr = buf.as_ptr();
    let start_ptr = buf_ptr;
    let end_ptr = unsafe { buf_ptr.add(buf_len) };
    ...
    if buf_len >= 32 {
        ...
        // to a aligned pointer
        {
            if !buf_ptr.is_aligned_u256() {
                ...
                #[cfg(feature = "test_alignment_check")]
                {
                    let r = basic::_nechr_dbl_to_aligned_u256(buf_ptr, needle, start_ptr);
                    if let Some(p) = r.0 {
                        buf_ptr = p;
                    } else if let Some(v) = r.1 {
                        return Some(v);
...
#[inline(always)]
fn _return_nechr_dbl<T, PU>(base: T, mask_a: PU) -> Option<usize>
where

...
#[inline(always)]
pub(crate) fn _memnechr_tpl_impl(buf: &[u8], needle: B1Tpl) -> Option<usize> {
...
unsafe fn _memnechr_tpl_basic(buf: &[u8], needle: B1Tpl) -> Option<usize> {
    basic::_memnechr_tpl_impl(buf, needle)
}

#[cfg(any(target_arch = "x86_64", target_arch = "x86"))]
#[target_feature(enable = "sse2")]
#[allow(clippy::missing_safety_doc)]
pub unsafe fn _memnechr_tpl_sse2(buf: &[u8], needle: B1Tpl) -> Option<usize> {
    _memnechr_tpl_sse2_impl(buf, needle)
}

#[cfg(any(target_arch = "x86_64", target_arch = "x86"))]
#[target_feature(enable = "avx2")]
#[allow(clippy::missing_safety_doc)]
pub unsafe fn _memnechr_tpl_avx2(buf: &[u8], needle: B1Tpl) -> Option<usize> {
    _memnechr_tpl_avx2_impl(buf, needle)
}

#[inline(always)]
fn _memnechr_tpl_sse2_impl(buf: &[u8], needle: B1Tpl) -> Option<usize> {
    let buf_len = buf.len();
    ...
    if buf_len >= 16 {
        ...
        // to a aligned pointer
        {
            if !buf_ptr.is_aligned_u128() {
                ...
                {
                    let r = basic::_nechr_tpl_to_aligned_u128(buf_ptr, needle, start_ptr);
                    if let Some(p) = r.0 {
                        buf_ptr = p;
        ...
        // the loop
        {
            let (r, p) = _unroll_loop_with_prefetch::<4, 16, _>(buf_ptr, end_ptr, |p| {
                unsafe { _nechr_c16_aa_x1(p, cc, start_ptr) }
            });
            if r.is_some() {
                return r;
            }
    ...
    basic::_memnechr_tpl_remaining_15_bytes_impl(buf_ptr, cc, start_ptr, end_ptr)
}

#[inline(always)]
fn _memnechr_tpl_avx2_impl(buf: &[u8], needle: B1Tpl) -> Option<usize> {
    let buf_len = buf.len();
    let mut buf_ptr = buf.as_ptr();
    let start_ptr = buf_ptr;
    let end_ptr = unsafe { buf_ptr.add(buf_len) };
    ...
    if buf_len >= 32 {
        ...
        // to a aligned pointer
        {
            if !buf_ptr.is_aligned_u256() {
                ...
                #[cfg(feature = "test_alignment_check")]
                {
                    let r = basic::_nechr_tpl_to_aligned_u256(buf_ptr, needle, start_ptr);
                    if let Some(p) = r.0 {
                        buf_ptr = p;
                    } else if let Some(v) = r.1 {
                        return Some(v);
        ...
        // the loop
        {
            let (r, p) = _unroll_loop::<1, 32, _>(buf_ptr, end_ptr, |p| {
                unsafe { _nechr_c32_aa_x1(p, cc, start_ptr) }
            });
            if r.is_some() {
                return r;
            }
    ...
    } else if buf_len >= 16 {
        {
            ...
            if buf_ptr.is_aligned_u128() {
                ...
                {
                    let (r, p) = _unroll_loop::<1, 16, _>(buf_ptr, end_ptr, |p| {
                        unsafe { _nechr_c16_uu_x1(p, cc, start_ptr) }
                    });
                    if r.is_some() {
                        return r;
                    }
...
#[inline(always)]
fn _return_nechr_tpl<T, PU>(base: T, mask_a: PU) -> Option<usize>
where
...
unsafe fn _nechr_c16_uu_x1(
    ...
    let mask_0_abc = unsafe {
        ...
        _mm_movemask_epi8(mm_0_eq_abc) as u16
    };
    let base = buf_ptr.usz_offset_from(st_ptr);
    //
    _return_nechr_tpl(base, mask_0_abc)
}
...
unsafe fn _nechr_c32_uu_x1(
    ...
    let mask_0_abc = unsafe {
        ...
        _mm256_movemask_epi8(mm_0_eq_abc) as u32
    };
    let base = buf_ptr.usz_offset_from(st_ptr);
    //
    _return_nechr_tpl(base, mask_0_abc)

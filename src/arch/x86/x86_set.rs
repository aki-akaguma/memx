...
fn _memset_avx2_impl(buf: &mut [u8], c1: u8) {
    ...
    if buf_len >= 32 {
        ...
        // the loop
        {
            buf_ptr = _unroll_loop_action::<8, 32, _>(buf_ptr, end_ptr, |p| {
                unsafe { _set_c32_aa_x1(p, cc) };
            });
        }
        {
            buf_ptr = _unroll_loop_action::<1, 32, _>(buf_ptr, end_ptr, |p| {
                unsafe { _set_c32_aa_x1(p, cc) };
            });
        }
        {

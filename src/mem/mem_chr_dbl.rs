...
#[inline(always)]
fn _start_chr_128(buf: &[u8], needle: B1Dbl) -> Option<usize> {
    let buf_len = buf.len();
    ...
    if buf_len >= 16 {
        // to a aligned pointer
        {
            if !buf_ptr.is_aligned_u128() {
                ...
                {
                    let r = _chr_dbl_c16_uu_x1(buf_ptr, cc, start_ptr);
                    if r.is_some() {
                        return r;
                    }
                    let remaining_align = 0x10_usize - ((buf_ptr as usize) & 0x0F_usize);
                ...
                {
                    let r = _chr_dbl_to_aligned_u128(buf_ptr, needle, start_ptr);
                    if let Some(p) = r.0 {
                        buf_ptr = p;

...
#[inline(always)]
fn _start_chr_128(buf: &[u8], needle: B1Qpl) -> Option<usize> {
    let buf_len = buf.len();
    ...
    if buf_len >= 16 {
        // to a aligned pointer
        {
            if !buf_ptr.is_aligned_u128() {
                ...
                {
                    let r = _chr_qpl_c16_uu_x1(buf_ptr, cc, start_ptr);
                    if r.is_some() {
                        return r;
                    }
                    let remaining_align = 0x10_usize - ((buf_ptr as usize) & 0x0F_usize);
                ...
                {
                    let c = B1Qpl::new(c_1, c_2);
                    let r = _chr_qpl_to_aligned_u128(buf_ptr, c, start_ptr);
                    if let Some(p) = r.0 {
                        buf_ptr = p;
                ...
                */
                let r = _chr_qpl_to_aligned_u128(buf_ptr, needle, start_ptr);
                if let Some(p) = r.0 {
                    buf_ptr = p;
                } else if let Some(v) = r.1 {
                    return Some(v);
        ...
        // the loop
        {
            ...
            while buf_ptr.is_not_over(end_ptr, loop_size * unroll) {
                buf_ptr.prefetch_read_data();
                let r = _chr_qpl_c16_aa_x8(buf_ptr, cc, start_ptr);
                if r.is_some() {
                    return r;
                }
        ...
        /*
        {
            ...
            while buf_ptr.is_not_over(end_ptr, loop_size * unroll) {
                buf_ptr.prefetch_read_data();
                let r = _chr_qpl_c16_aa_x4(buf_ptr, cc, start_ptr);
                if r.is_some() {
                    return r;
                }
                buf_ptr = unsafe { buf_ptr.add(loop_size * unroll) };
        ...
        }
        {
            ...
            while buf_ptr.is_not_over(end_ptr, loop_size * unroll) {
                buf_ptr.prefetch_read_data();
                let r = _chr_qpl_c16_aa_x2(buf_ptr, cc, start_ptr);
                if r.is_some() {
                    return r;
                }
                buf_ptr = unsafe { buf_ptr.add(loop_size * unroll) };
        ...
        */
        {
            ...
            while buf_ptr.is_not_over(end_ptr, loop_size * unroll) {
                buf_ptr.prefetch_read_data();
                let r = _chr_qpl_c16_aa_x1(buf_ptr, cc, start_ptr);
                if r.is_some() {
                    return r;
                }
                buf_ptr = unsafe { buf_ptr.add(loop_size * unroll) };
    ...
    _memchr_qpl_remaining_15_bytes_impl(buf_ptr, cc.into(), start_ptr, end_ptr)
}
    ...
    if buf_len >= 8 {
        // to a aligned pointer
        {
            if !buf_ptr.is_aligned_u64() {
                ...
                {
                    let c = B1Qpl::new(c_1, c_2);
                    let r = _chr_qpl_to_aligned_u64(buf_ptr, c, start_ptr);
                    if let Some(p) = r.0 {
                        buf_ptr = p;
                    } else if let Some(v) = r.1 {
                        return Some(v);
    ...
    if buf_len >= 4 {
        // to a aligned pointer
        {
            if !buf_ptr.is_aligned_u32() {
                ...
                {
                    let c = B1Qpl::new(c_1, c_2);
                    let r = _chr_qpl_to_aligned_u32(buf_ptr, c, start_ptr);
                    if let Some(p) = r.0 {
                        buf_ptr = p;
                    } else if let Some(v) = r.1 {
                        return Some(v);
...
#[inline(always)]
pub(crate) fn _memchr_qpl_remaining_3_bytes_impl(
    buf_ptr: *const u8,
    cc: B2Qpl,
    start_ptr: *const u8,
    end_ptr: *const u8,

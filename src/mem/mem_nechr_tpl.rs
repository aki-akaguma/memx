...
#[inline(always)]
fn _start_nechr_128(buf: &[u8], needle: B1Tpl) -> Option<usize> {
    let buf_len = buf.len();
    ...
    if buf_len >= 16 {
        // to a aligned pointer
        {
            if !buf_ptr.is_aligned_u128() {
                ...
                {
                    let r = _nechr_tpl_to_aligned_u128(buf_ptr, needle, start_ptr);
                    if let Some(p) = r.0 {
                        buf_ptr = p;
    ...
    _memnechr_tpl_remaining_15_bytes_impl(buf_ptr, cc.into(), start_ptr, end_ptr)
}

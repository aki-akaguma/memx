...
#[inline(always)]
fn _rchr_sgl_to_aligned<const ALIGN: usize>(
    ...
    while cur_ptr > buf_ptr_min {
        ...
        if let Some(pos) = _rchr_sgl_c1_aa_x1(cur_ptr, c, st_ptr) {
...
pub(crate) fn _rchr_sgl_to_aligned_u256(
    ...
    _rchr_sgl_to_aligned::<32>(buf_ptr_cur, c, st_ptr)
}
...
pub(crate) fn _rchr_sgl_to_aligned_u128(
    ...
    _rchr_sgl_to_aligned::<16>(buf_ptr_cur, c, st_ptr)
}
...
fn _rchr_sgl_to_aligned_u64(
    ...
    _rchr_sgl_to_aligned::<8>(buf_ptr_cur, c, st_ptr)
}
...
fn _rchr_sgl_to_aligned_u32(
    ...
    _rchr_sgl_to_aligned::<4>(buf_ptr_cur, c, st_ptr)
}
    ...
    if buf_len >= 8 {
        ...
        // the loop
        {
            let (r, p) = _unroll_loop_backward_with_prefetch::<8, 8, _>(buf_ptr, start_ptr, |p| {
                _rchr_sgl_c8_aa_x1(p, cc, start_ptr)
            });
            if r.is_some() {
                return r;
            }
    ...
    if buf_len >= 4 {
        ...
        // the loop
        {
            let (r, p) = _unroll_loop_backward::<4, 4, _>(buf_ptr, start_ptr, |p| {
                _rchr_sgl_c4_aa_x1(p, cc, start_ptr)
            });
            if r.is_some() {
                return r;
            }
...
fn _rchr_sgl_c16_aa_x2(buf_ptr: *const u8, c16: B16Sgl, st_ptr: *const u8) -> Option<usize> {
    let (r, _) = _unroll_loop_backward::<2, 16, _>(unsafe { buf_ptr.add(16 * 2) }, buf_ptr, |p| {
        _rchr_sgl_c16_aa_x1(p, c16, st_ptr)
    });
    r
}
...
fn _rchr_sgl_c8_aa_x2(buf_ptr: *const u8, c8: B8Sgl, st_ptr: *const u8) -> Option<usize> {
    let (r, _) = _unroll_loop_backward::<2, 8, _>(unsafe { buf_ptr.add(8 * 2) }, buf_ptr, |p| {
        _rchr_sgl_c8_aa_x1(p, c8, st_ptr)
    });
    r
}
...
fn _rchr_sgl_c4_aa_x2(buf_ptr: *const u8, c4: B4Sgl, st_ptr: *const u8) -> Option<usize> {
    let (r, _) = _unroll_loop_backward::<2, 4, _>(unsafe { buf_ptr.add(4 * 2) }, buf_ptr, |p| {
        _rchr_sgl_c4_aa_x1(p, c4, st_ptr)
    });
    r
}

...
#[cfg(any(target_pointer_width = "64", feature = "test_pointer_width_64"))]
#[inline(always)]
fn _start_nechr_dbl_64(buf: &[u8], needle: B1Dbl) -> Option<usize> {
    let buf_len = buf.len();
    ...
    if buf_len >= 8 {
        // to a aligned pointer
        {
            if !buf_ptr.is_aligned_u64() {
                let r = _nechr_dbl_to_aligned_u64(buf_ptr, needle, start_ptr);
                if let Some(p) = r.0 {
                    buf_ptr = p;
        ...
        // the loop
        {
            let (r, p) = _unroll_loop_with_prefetch::<4, 8, _>(buf_ptr, end_ptr, |p| {
                _nechr_dbl_c8_aa_x1(p, cc, start_ptr)
            });
            if r.is_some() {
                return r;
            }
    ...
    _memnechr_dbl_remaining_7_bytes_impl(buf_ptr, cc.into(), start_ptr, end_ptr)
}

#[cfg(any(target_pointer_width = "32", feature = "test_pointer_width_32"))]
#[inline(always)]
fn _start_nechr_dbl_32(buf: &[u8], needle: B1Dbl) -> Option<usize> {
    let buf_len = buf.len();
    let mut buf_ptr = buf.as_ptr();
    let start_ptr = buf_ptr;
    let end_ptr = unsafe { buf_ptr.add(buf_len) };
    ...
    if buf_len >= 4 {
        // to a aligned pointer
        {
            if !buf_ptr.is_aligned_u32() {
                let r = _nechr_dbl_to_aligned_u32(buf_ptr, needle, start_ptr);
                if let Some(p) = r.0 {
                    buf_ptr = p;
                } else if let Some(v) = r.1 {
                    return Some(v);
        ...
        // the loop
        {
            let (r, p) = _unroll_loop_with_prefetch::<4, 4, _>(buf_ptr, end_ptr, |p| {
                _nechr_dbl_c4_aa_x1(p, cc, start_ptr)
            });
            if r.is_some() {
                return r;
            }
    ...
    _memnechr_dbl_remaining_3_bytes_impl(buf_ptr, cc.into(), start_ptr, end_ptr)
}

#[inline(always)]
pub(crate) fn _memnechr_dbl_remaining_15_bytes_impl(
    buf_ptr: *const u8,
    cc: B8Dbl,
    start_ptr: *const u8,
    end_ptr: *const u8,
    ...
    if buf_ptr.is_aligned_u64() {
        ...
        if buf_ptr.is_not_over(end_ptr, loop_size) {
            let r = _nechr_dbl_c8_aa_x1(buf_ptr, cc, start_ptr);
            if r.is_some() {
                return r;
            }
...
#[inline(always)]
pub(crate) fn _memnechr_dbl_remaining_7_bytes_impl(
    buf_ptr: *const u8,
    cc: B4Dbl,
    start_ptr: *const u8,
    end_ptr: *const u8,
    ...
    if buf_ptr.is_aligned_u32() {
        let loop_size = 4;
        if buf_ptr.is_not_over(end_ptr, loop_size) {
            let r = _nechr_dbl_c4_aa_x1(buf_ptr, cc, start_ptr);
            if r.is_some() {
                return r;
            }
            buf_ptr = unsafe { buf_ptr.add(loop_size) };
...
#[inline(always)]
fn _memnechr_dbl_remaining_3_bytes_impl(
    buf_ptr: *const u8,
    cc: B2Dbl,
    start_ptr: *const u8,
    end_ptr: *const u8,
    ...
    if buf_ptr.is_aligned_u16() {
        let loop_size = 2;
        if buf_ptr.is_not_over(end_ptr, loop_size) {
            let r = _nechr_dbl_c2_aa_x1(buf_ptr, cc, start_ptr);
            if r.is_some() {
                return r;
            }
            buf_ptr = unsafe { buf_ptr.add(loop_size) };
    ...
    }
    {
        ...
        while buf_ptr.is_not_over(end_ptr, loop_size) {
            let r = _nechr_dbl_c1_aa_x1(buf_ptr, cc.into(), start_ptr);
            if r.is_some() {
                return r;
            }
            buf_ptr = unsafe { buf_ptr.add(loop_size) };
...
#[inline(always)]
fn _return_nechr_dbl<T, PU>(base: T, bits_ab: PU) -> Option<usize>
where
...
#[inline(always)]
fn _nechr_dbl_c16_uu_x1(buf_ptr: *const u8, c16: B16Dbl, st_ptr: *const u8) -> Option<usize> {
    _nechr_dbl_c16_aa_x1(buf_ptr, c16, st_ptr)
}

#[inline(always)]
fn _nechr_dbl_c16_aa_x1(buf_ptr: *const u8, c16: B16Dbl, st_ptr: *const u8) -> Option<usize> {
    ...
    //
    _return_nechr_dbl(base, bits_0_ab)
}

#[inline(always)]
fn _nechr_dbl_c16_aa_x2(buf_ptr: *const u8, c16: B16Dbl, st_ptr: *const u8) -> Option<usize> {
    let (r, _) = _unroll_loop::<2, 16, _>(buf_ptr, unsafe { buf_ptr.add(16 * 2) }, |p| {
        _nechr_dbl_c16_aa_x1(p, c16, st_ptr)
    });
    r
}

#[inline(always)]
fn _nechr_dbl_c16_aa_x4(buf_ptr: *const u8, c16: B16Dbl, st_ptr: *const u8) -> Option<usize> {
...
#[inline(always)]
fn _nechr_dbl_c16_aa_x8(buf_ptr: *const u8, c16: B16Dbl, st_ptr: *const u8) -> Option<usize> {
...
#[inline(always)]
fn _nechr_dbl_c8_uu_x1(buf_ptr: *const u8, c8: B8Dbl, st_ptr: *const u8) -> Option<usize> {
    _nechr_dbl_c8_aa_x1(buf_ptr, c8, st_ptr)
}

#[inline(always)]
fn _nechr_dbl_c8_aa_x1(buf_ptr: *const u8, c8: B8Dbl, st_ptr: *const u8) -> Option<usize> {
...
#[inline(always)]
fn _nechr_dbl_c8_aa_x2(buf_ptr: *const u8, c8: B8Dbl, st_ptr: *const u8) -> Option<usize> {
    let (r, _) = _unroll_loop::<2, 8, _>(buf_ptr, unsafe { buf_ptr.add(8 * 2) }, |p| {
        _nechr_dbl_c8_aa_x1(p, c8, st_ptr)
    });
    r
}

#[inline(always)]
fn _nechr_dbl_c8_aa_x4(buf_ptr: *const u8, c8: B8Dbl, st_ptr: *const u8) -> Option<usize> {
...
#[inline(always)]
fn _nechr_dbl_c8_aa_x8(buf_ptr: *const u8, c8: B8Dbl, st_ptr: *const u8) -> Option<usize> {
...
#[inline(always)]
fn _nechr_dbl_c4_uu_x1(buf_ptr: *const u8, c4: B4Dbl, st_ptr: *const u8) -> Option<usize> {
    _nechr_dbl_c4_aa_x1(buf_ptr, c4, st_ptr)
}

#[inline(always)]
fn _nechr_dbl_c4_aa_x1(buf_ptr: *const u8, c4: B4Dbl, st_ptr: *const u8) -> Option<usize> {
...
#[inline(always)]
fn _nechr_dbl_c4_aa_x2(buf_ptr: *const u8, c4: B4Dbl, st_ptr: *const u8) -> Option<usize> {
    let (r, _) = _unroll_loop::<2, 4, _>(buf_ptr, unsafe { buf_ptr.add(4 * 2) }, |p| {
        _nechr_dbl_c4_aa_x1(p, c4, st_ptr)
    });
    r
}

#[inline(always)]
fn _nechr_dbl_c4_aa_x4(buf_ptr: *const u8, c4: B4Dbl, st_ptr: *const u8) -> Option<usize> {
...
#[inline(always)]
fn _nechr_dbl_c4_aa_x8(buf_ptr: *const u8, c4: B4Dbl, st_ptr: *const u8) -> Option<usize> {
...
#[inline(always)]
fn _nechr_dbl_c2_aa_x1(buf_ptr: *const u8, c2: B2Dbl, st_ptr: *const u8) -> Option<usize> {
...
#[inline(always)]
fn _nechr_dbl_c1_aa_x1(buf_ptr: *const u8, c1: B1Dbl, st_ptr: *const u8) -> Option<usize> {
    let aa_ptr = buf_ptr;
...
fn _nechr_dbl_to_aligned<const ALIGN: usize>(
    ...
    while cur_ptr < buf_ptr_end {
        if let Some(pos) = _nechr_dbl_c1_aa_x1(cur_ptr, c, start_ptr) {
...
#[inline(always)]
pub(crate) fn _nechr_dbl_to_aligned_u256(
    buf_ptr: *const u8,
    c: B1Dbl,
    start_ptr: *const u8,
) -> (Option<*const u8>, Option<usize>) {
...
#[inline(always)]
pub(crate) fn _nechr_dbl_to_aligned_u128(
    buf_ptr: *const u8,
    c: B1Dbl,
    start_ptr: *const u8,
) -> (Option<*const u8>, Option<usize>) {

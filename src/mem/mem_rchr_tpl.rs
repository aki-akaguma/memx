    ...
    if buf_len >= 8 {
        ...
        // the loop
        {
            let (r, p) = _unroll_loop_backward_with_prefetch::<4, 8, _>(buf_ptr, min_ptr, |p| {
                _rchr_tpl_c8_aa_x1(p, cc, start_ptr)
            });
            if r.is_some() {
                return r;
            }
    ...
    if buf_len >= 4 {
        ...
        // the loop
        {
            let (r, p) = _unroll_loop_backward_with_prefetch::<4, 4, _>(buf_ptr, min_ptr, |p| {
                _rchr_tpl_c4_aa_x1(p, cc, start_ptr)
            });
            if r.is_some() {
                return r;
            }
...
#[inline(always)]
fn _memrchr_tpl_remaining_7_bytes_impl(
    buf_ptr: *const u8,
    cc: B4Tpl,
    start_ptr: *const u8,
) -> Option<usize> {
...
fn _rchr_tpl_c16_aa_x2(buf_ptr: *const u8, c16: B16Tpl, st_ptr: *const u8) -> Option<usize> {
    let (r, _) = _unroll_loop_backward::<2, 16, _>(unsafe { buf_ptr.add(16 * 2) }, buf_ptr, |p| {
        _rchr_tpl_c16_aa_x1(p, c16, st_ptr)
    });
    r
}
...
fn _rchr_tpl_c8_aa_x2(buf_ptr: *const u8, c8: B8Tpl, st_ptr: *const u8) -> Option<usize> {
    let (r, _) = _unroll_loop_backward::<2, 8, _>(unsafe { buf_ptr.add(8 * 2) }, buf_ptr, |p| {
        _rchr_tpl_c8_aa_x1(p, c8, st_ptr)
    });
    r
}
...
fn _rchr_tpl_c4_aa_x2(buf_ptr: *const u8, c4: B4Tpl, st_ptr: *const u8) -> Option<usize> {
    let (r, _) = _unroll_loop_backward::<2, 4, _>(unsafe { buf_ptr.add(4 * 2) }, buf_ptr, |p| {
        _rchr_tpl_c4_aa_x1(p, c4, st_ptr)
    });
    r
}

...
#[inline(always)]
fn _start_rchr_128(buf: &[u8], needle: B1Qpl) -> Option<usize> {
    let buf_len = buf.len();
    ...
    if buf_len >= 16 {
        // to a aligned pointer
        {
            if !buf_ptr.is_aligned_u128() {
                let r = _rchr_qpl_to_aligned_u128(buf_ptr, needle, start_ptr);
                if let Some(p) = r.0 {
                    buf_ptr = p;
        ...
        /*
        {
            ...
            while buf_ptr.is_not_under(start_ptr, loop_size * unroll) {
                ...
                let r = _rchr_qpl_c16_aa_x8(buf_ptr, cc, start_ptr);
                if r.is_some() {
                    return r;
                }
            }
        ...
        */
        {
            ...
            while buf_ptr.is_not_under(start_ptr, loop_size * unroll) {
                buf_ptr = unsafe { buf_ptr.sub(loop_size * unroll) };
                let r = _rchr_qpl_c16_aa_x4(buf_ptr, cc, start_ptr);
                if r.is_some() {
                    return r;
                }
            }
        ...
        /*
        {
            ...
            while buf_ptr.is_not_under(start_ptr, loop_size * unroll) {
                buf_ptr = unsafe { buf_ptr.sub(loop_size * unroll) };
                let r = _rchr_qpl_c16_aa_x2(buf_ptr, cc, start_ptr);
                if r.is_some() {
                    return r;
                }
            }
        ...
        */
        {
            ...
            while buf_ptr.is_not_under(start_ptr, loop_size * unroll) {
                buf_ptr = unsafe { buf_ptr.sub(loop_size * unroll) };
                let r = _rchr_qpl_c16_aa_x1(buf_ptr, cc, start_ptr);
                if r.is_some() {
                    return r;
                }
            }
    ...
    _memrchr_qpl_remaining_15_bytes_impl(buf_ptr, cc.into(), start_ptr)
}
...
#[inline(always)]
pub(crate) fn _memrchr_qpl_remaining_3_bytes_impl(
    buf_ptr: *const u8,
    cc: B2Qpl,
    start_ptr: *const u8,
) -> Option<usize> {
